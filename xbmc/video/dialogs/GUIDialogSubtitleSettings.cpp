...
void CGUIDialogSubtitleSettings::OnSettingAction(const CSetting *setting)
{
  ...
  {
    CGUIDialog *dialog = (CGUIDialog *)g_windowManager.GetWindow(WINDOW_DIALOG_SUBTITLES);
    if (dialog)
    {
      dialog->Open();
...
void CGUIDialogSubtitleSettings::InitializeSettings()
{
  ...
  if (g_application.m_pPlayer->HasPlayer())
  {
    g_application.m_pPlayer->GetSubtitleCapabilities(m_subtitleCapabilities);
  }
  ...
  AddToggle(groupSubtitles, SETTING_SUBTITLE_ENABLE, 13397, SettingLevel::SettingLevelBasic, m_subtitleVisible);

  // subtitle delay setting
  if (SupportsSubtitleFeature(IPC_SUBS_OFFSET))
  {
    CSettingNumber *settingSubtitleDelay = AddSlider(groupSubtitles, SETTING_SUBTITLE_DELAY, 22006, SettingLevel::SettingLevelBasic, videoSettings.m_SubtitleDelay, 0, -g_advancedSettings.m_videoSubsDelayRange, 0.1f, g_advancedSettings.m_videoSubsDelayRange, 22006, usePopup);
  ...
  if (SupportsSubtitleFeature(IPC_SUBS_EXTERNAL))
    AddButton(groupSubtitles, SETTING_SUBTITLE_BROWSER, 13250, SettingLevel::SettingLevelBasic);
  //downloading subtitle
  AddButton(groupSubtitles, SETTING_SUBTITLE_SEARCH, 24153, SettingLevel::SettingLevelBasic);

  // subtitle stream setting
  AddButton(groupSaveAsDefault, SETTING_MAKE_DEFAULT, 12376, SettingLevel::SettingLevelBasic);
}
...
void CGUIDialogSubtitleSettings::AddSubtitleStreams(CSettingGroup *group, const std::string &settingId)
{
  ...
  m_subtitleStreamSetting = AddList(group, settingId, 462, SettingLevel::SettingLevelBasic, m_subtitleStream, SubtitleStreamsOptionFiller, 462);
}
